...
izip = zip
from facecog.helper import nms, adjust_input, generate_bbox, detect_first_stage_warpper
...
class MtcnnDetector(object):
    ...
    def detect_face(self, img):
        ...
        print('sliced_index = ', len(sliced_index))
        total_boxes = []
        for batch in sliced_index:
            print('in')
            print('izip = ', len(list(zip(repeat(img), self.PNets[:len(batch)], [scales[i] for i in batch], repeat(self.threshold[0])))))
